...
func wireApp(bootstrap *conf.Bootstrap, logger *zap.Logger) (*kratos.App, func(), error) {
	...
	fileRepo := data.NewFileRepo(dataData, baseRepo)
	fileUseCase := biz.NewFileUseCase(fileRepo)
	fileService := service.NewFileService(fileUseCase)
	grpcServer := server.NewGRPCServer(bootstrap, jwtHelper, enforcer, userService, roleService, permissionService, fileService)
